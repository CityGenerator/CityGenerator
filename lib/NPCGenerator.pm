...
sub set_class{
    ...
    $npc->{'class'} = roll_from_array($npc->{'class_roll'},   $npc_data->{'classes'}->{'option'} )->{'type'} if (!defined $npc->{'class'});
...
sub set_motivation{
    ...
    $npc->{'motivation'} = rand_from_array( [ keys %{ $npc_data->{'motivation'}->{'motive'} } ]  ) if (!defined $npc->{'motivation'});
    ...
    if (defined $npc_data->{'motivation'}->{'motive'}->{  $npc->{'motivation'} }){
        ...
        $npc->{'motivation_detail'} = rand_from_array( $npc_data->{'motivation'}->{'motive'}->{  $npc->{'motivation'}  }->{'option'})->{'content'}  if (!defined $npc->{'motivation_detail'});
    }else{
