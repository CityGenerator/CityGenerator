...
subtest 'test create' => sub {
    my $item;
    $item = MagicItemGenerator::create( );
    isnt( $item->{'seed'}, undef, 'ensure seed is set.' );

    $item = MagicItemGenerator::create( { 'seed' => 12 } );
...
subtest 'test itemtypes' => sub {
    my $item;
    $item = MagicItemGenerator::create( { 'seed' => 12, 'item'=>'armor' } );
    is( $item->{'seed'}, 12, 'ensure seed is set.' );
...
subtest 'test item stats' => sub {
    ...
    $item = MagicItemGenerator::create( { 'seed' => 12, 'item'=>'potion', 'stats'=>$stats,   } );
    ...
    $item = MagicItemGenerator::create( $presets );
