...
subtest 'test create' => sub {
    ...
    $military = MilitaryGenerator::create();
    is( $military->{'seed'},          41630 );

    $military = MilitaryGenerator::create( { 'seed' => 22 } );
    is( $military->{'seed'},          22 );
...
subtest 'test generate_fortification' => sub {
    my $military;
    $military = MilitaryGenerator::create( { 'seed' => 41630 } );
    isnt( $military->{'fortification_roll'}, undef, 'fort roll is set' );
    isnt( $military->{'fortification'},      undef, 'fort is created'  );

    $military = MilitaryGenerator::create( { 'seed' => 22, 'fortification'=>'foo' } );
    is( $military->{'fortification'},      'foo' );

    $military = MilitaryGenerator::create( { 'seed' => 22, 'fortification_roll'=>100 } );
    is( $military->{'fortification_roll'}, '100' );
    is( $military->{'fortification'},      'perfectly' );

    $military = MilitaryGenerator::create( { 'seed' => 22,'fortification_roll'=>0 } );
...
subtest 'test generate_favored_tactic' => sub {
    ...
    $military = MilitaryGenerator::create( { 'seed' => 41630, 'favored tactic'=>'spitwads' } );
    $military = MilitaryGenerator::generate_favored_tactic($military);
...
subtest 'test generate_reputation' => sub {
    ...
    $military = MilitaryGenerator::create( { 'seed' => 41630, 'reputation'=>'spitwads' } );
...
subtest 'test generate_favored_weapon' => sub {
    ...
    $military = MilitaryGenerator::create( { 'seed' => 41630,'favored weapon' => "shoes" } );
...
subtest 'test set_troop_size' => sub {
    ...
    $military = MilitaryGenerator::create( { 'seed' => 1, 'population_total' => 10000, } );
    foreach my $value (qw( seed population_total active_percent reserve_percent para_percent active_troops reserve_troops para_troops)){
        isnt($military->{$value}, undef, "ensure $value is set");
    }

    my $military2
        = MilitaryGenerator::create( { 'seed' => 1, 'population_total' => 10000, 'military_mod' => 5, } );
    ...
    $military2
        = MilitaryGenerator::create(       { 'seed' => 1, 'population_total' => 10000, 'authority_mod' => 5, } );
    is( $military2->{'seed'},             $military->{'seed'},           "ensure seed is the same" );
    ok( $military2->{'active_percent'}   >$military->{'active_percent'}, "ensure active_percent is larger");
    ok( $military2->{'reserve_percent'}  >$military->{'reserve_percent'},"ensure reserve_percent is larger");
    #ok( $military2->{'para_percent'}     >$military->{'para_percent'},   "ensure para_percent is larger");
    ...
    $military2 = MilitaryGenerator::create( { 'seed' => 1, 'population_total' => 10000, 'military_mod' => 5, 'authority_mod' => 5 } );
    is( $military2->{'seed'},             $military->{'seed'},           "ensure seed is the same" );
    ok( $military2->{'active_percent'}   >$military->{'active_percent'}, "ensure active_percent is larger");
    ok( $military2->{'reserve_percent'}  >$military->{'reserve_percent'},"ensure reserve_percent is larger");
    #ok( $military2->{'para_percent'}     >$military->{'para_percent'},   "ensure para_percent is larger");
    ...
    $military2 = MilitaryGenerator::create( { 'seed' => 1, 'population_total' => 10000, 'active_percent' => 0,'reserve_percent'=>0,'para_percent'=>0 } );
    is( $military2->{'seed'},             $military->{'seed'},           "ensure seed is the same" );
