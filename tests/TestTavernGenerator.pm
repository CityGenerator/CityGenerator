...
subtest 'test create_tavern' => sub {
    ...
    is($tavern->{'name'},'Royal Harpoon');
    is($tavern->{'stats'}->{'cost'},47);
    is($tavern->{'stats'}->{'popularity'},13);
    is($tavern->{'stats'}->{'size'},15);
    is($tavern->{'stats'}->{'reputation'},53);
...
subtest 'test generate_bartender' => sub {
    ...
    is($tavern->{'bartender'}->{'race'},'minotaur');
...
subtest 'test generate_amenities' => sub {
    ...
    $tavern=TavernGenerator::create_tavern({'seed'=>22});
    is($tavern->{'amenity_count'} ,0);
    is(scalar(@{$tavern->{'amenity'} }) ,0);
